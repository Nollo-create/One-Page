  ...
  return (
    <section id="work" className="section" style={{ backgroundColor: 'var(--bg)' }}>
      <div className="container">
        ...
        {/* Featured */}
        <ProjectCard project={featured} aspectRatio="50%" large delay={0} />

        {/* 3-col grid */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px', marginTop: '16px' }}>
          {rest.map((p, i) => (
            <ProjectCard key={p.id} project={p} aspectRatio="72%" large={false} delay={i + 1} />
          ))}
        </div>
      </div>
    </section>
...
function ProjectCard({
  ...
  return (
    ...
    >
      ...
      >
        {/* Mockup — scales slightly on hover */}
        <div
          style={{
            position: 'absolute',
            inset: 0,
...
function PortakalMockup({ W, H }: { W: number; H: number }) {
  return (
    <>
      ...
      {/* Abstract wine element right side */}
...
function HanaMockup({ W, H }: { W: number; H: number }) {
  return (
    <>
      ...
      {/* Product grid */}
      {[[W*0.04, 70, '#C8A898'], [W*0.37, 70, '#B8987A'], [W*0.7, 70, '#D4B8A8']].map(([x, y, c], i) => (
        <g key={i}>
          <rect x={x as number} y={y as number} width={W * 0.28} height={H * 0.48} rx="4" fill={c as string} opacity=".4" />
          <rect x={(x as number) + 12} y={(y as number) + (H * 0.48) + 12} width={W * 0.14} height="7" rx="3" fill="#2A1A12" opacity=".25" />
          <rect x={(x as number) + 12} y={(y as number) + (H * 0.48) + 25} width={W * 0.09} height="6" rx="3" fill="#8C5C45" opacity=".4" />
        </g>
      ))}
